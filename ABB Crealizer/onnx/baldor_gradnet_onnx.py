...
# =====================================================================
# ONNX EXPORT WRAPPER (This ensures C++ sees the full math)
# =====================================================================
class GradNetOnnxWrapper(nn.Module):
    """
    This wrapper includes Normalization and De-normalization inside the ONNX file.
    Input: Real Flux (psi_d, psi_q)
    Output: Real Current (i_d, i_q)
    """
    def __init__(self, ai_model, psi_base, i_base):
        super().__init__()
        self.ai_model = ai_model
        # constants for the ONNX graph
        self.psi_base = psi_base
        self.i_base = i_base

    def forward(self, psi_input):
        # 1. Normalize 
        x = psi_input / self.psi_base
        # 2. AI Inference 
        out = self.ai_model(x)
        # 3. De-normalize
        return out * self.i_base
...
class BaselineDigitalTwinRK4GradNet:
    ...
    def export_onnx(self, filename="baldor_gradnet.onnx"):
        """Generates the ONNX file for ABB Crealizer / onnx2c."""
        wrapper = GradNetOnnxWrapper(self.ai_model, self.Psi_base, self.I_base)
        wrapper.eval()

        # Dummy input for the graph tracer [BatchSize=1, Inputs=2]
        ...
        torch.onnx.export(
            ...
            input_names=['flux_input'],      # Matches C++ input name
            output_names=['current_output']  # Matches C++ output name
        )
        print(f"ONNX model exported: {filename}")
...
if __name__ == "__main__":
    try:
        ...
        # 1. RUN THE EXPORT (This creates the file for C++ onnx2c step)
        twin.export_onnx("src/c++/onnx/baldor_gradnet.onnx")

        # 2. TEST STEP
        print("Testing Python Digital Twin Step...")
